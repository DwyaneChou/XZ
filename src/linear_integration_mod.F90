module linear_integration_mod
    ...
    subroutine poly5_integration(n,x,y,ty)
      integer, intent(in ) :: n
      real   , intent(in ) :: x (n)
      real   , intent(in ) :: y (n)
      real   , intent(out) :: ty(n)
      ...
      do k = 1,nt
        if(k==1)then
          nsample = 3
          xSample(1:nSample) = x(1:nSample)
          ySample(1:nSample) = y(1:nSample)
        elseif(k==nt)then
          nsample = 3
          xSample(1:nSample) = x(n-nSample+1:n)
          ySample(1:nSample) = y(n-nSample+1:n)
        elseif(k==2)then
          nsample = 4
          xSample(1:nSample) = x(1:nSample)
          ySample(1:nSample) = y(1:nSample)
        elseif(k==nt-1)then
          ...
          nsample = 5
          xSample(1:nSample) = x(k-2:k+2)
          ySample(1:nSample) = y(k-2:k+2)
        endif
        
        coef(1:nsample) = poly_coef(nsample,xSample(1:nsample),ySample(1:nsample))
        
        f(1) = intfunc(nSample,coef(1:nsample),x(k  ))
        f(2) = intfunc(nSample,coef(1:nsample),x(k+1))
        
        ty(k) = f(2) - f(1)
        !print*,k,ty(k)
        !print*,xSample(1:nsample)
        !print*,ySample(1:nsample)
        !print*,''
      enddo
      
    contains
      function poly_coef(np,x,f)
        real   ,dimension(np)            :: poly_coef
        integer              ,intent(in) :: np
        real   ,dimension(np),intent(in) :: x
        real   ,dimension(np),intent(in) :: f
        
        real,dimension(np,np) :: mtx
        real,dimension(np,np) :: imtx
        
        integer :: inverse_status
        
        integer iPoint,iterm
        
        do iPoint = 1,np
          do iterm = 1,np
            mtx(iPoint,iterm) = x(iPoint)**(iterm-1)
          enddo
        enddo
        
        call BRINV(np,mtx,imtx,inverse_status)
        if(inverse_status==0)then
          print*,'Inverse of poly matrix is not avaliable'
          stop
        endif
        
        poly_coef = matmul(imtx,f)
        
      end function poly_coef
      
      function intfunc(np,coef,x)
        real    intfunc
        integer np
        real    coef(np)
        real    x
        ...
        intfunc = 0
        do iterm = 1,np
          intfunc = intfunc + coef(iterm) / real(iterm) * x**iterm
        enddo
        
      end function intfunc
    end subroutine poly5_integration
